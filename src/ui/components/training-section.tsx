...
export function TrainingSection() {
  ...
  function computeNaiveScramble(f2l: string): string {
     ...
     * each scramble looks a bit diferent. There may be a better way to do this
     */
    const oll = sample(OLLS)!;
    const pll = sample(PLLS)!;

    /**
     * I chose to add random U moves as a sufix to the scramble so that the case
  ...
  return (
    <div onFocus={console.log} onBlur={console.log} className="w-3/4 m-auto mb-5">
...
function Scramble(props: ScrambleProps) {
  ...
  return (
    <span>
      ...
      <p className="text-3xl font-bold text-center">{props.scramble}</p>
    </span>
  );
}
